...
- 우상단 floating panel에 캡처 진행 상황(0/2 ~ 2/2 — 핵심 응답 `GetUnionRaidData` + `GetGuildMembers` 기준, 나머지 2개는 옵셔널 메타)과 진단 메시지(로그인 필요 / 데이터 없음 / 오류)를 표시합니다.
- (v2.5.0+) 캡처/회차 진행을 동반 도구(SPA) 화면에도 **미러링**하여, 새 탭으로 전환하지 않고도 도구에서 수집 진행을 확인할 수 있습니다.
...
- **수집 진행 미러링** (v2.5.0) — 캡처/회차 진행을 패널 갱신 시점에 `nra-progress` 메시지(`{type, captured, total:2, statusText, scriptVersion}`)로 동반 도구(SPA)에 동반 전송합니다. SPA가 이를 받아 화면에 수집 진행을 표시하므로, 새 탭으로 전환하지 않고도 진행 상황을 볼 수 있습니다. 기존 캡처/전송 코어는 무변경(`updatePanel` 지점에 비침습 추가)이며, opener 미연결·구버전 도구 조합에서도 graceful하게 무시됩니다.
